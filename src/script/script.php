<?php
	...
	$db->set_charset("utf8");  
	$statement = $db->prepare("INSERT INTO `upload` (`uploaderIP`, `reCAPTCHApass`, `imageDate`, `imageTime`, `imagePath`, `EXIFmanufacturer`, `EXIFmodel`, `gpsLat`, `gpsLong`, `gpsAlt`, `gpsTimestamp`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"); 
	if ($statement === false){die('prepare() failed: ' . htmlspecialchars($db->error));}
	$statement->bind_param("sisssssssds", $requestIP, $reCAPTCHApass, $date, $time, $path, $EXIFmanufacturer, $EXIFmodel, $EXIFlat, $EXIFlong, $EXIFalt, $EXIFtimestamp); 
	$statement->execute(); 
?>